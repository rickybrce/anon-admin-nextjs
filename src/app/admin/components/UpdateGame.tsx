...
import { updateGame } from "@/app/api/games";
import DatePicker from "react-datepicker";
import "react-datepicker/dist/react-datepicker.css";

type Props = {
  game?: any;
  handleUGame?: Function;
};

const UpdateGame = ({ game, handleUGame }: Props) => {
  ...
  const [betsSoftClose, setBetsSoftClose] = useState("");
  const [betsHardClose, setBetsHardClose] = useState("");
  ...
  useEffect(() => {
    setName(game.name);
    setOpenDate(game.open_date);
    setCloseDate(game.close_date);
    setBetsSoftClose(game.bets_soft_close);
    setBetsHardClose(game.bets_hard_close);
    setEnterAmount(game.enter_amount);
    setRemoveBetAmount(game.remove_bet_amount);
    setBetLateAmount(game.bet_late_amount);
    setExtraCoinsAmount(game.extra_coins_amount);
    setExtraLevAmount(game.extra_lev_amount);
    setRevealBetAmount(game.reveal_bet_amount);
    setInitialAmount(game.initial_amount);
    setCategory1Name(game.category1_name);
    setCategory2Name(game.category2_name);
    setCategory3Name(game.category3_name);
    setCategory4Name(game.category4_name);
        ...
        <>
          ...
          <div className="absolute right-4 top-12 z-10">
              <DatePicker
                ...
                selected={openDate ? new Date(openDate) : null}
          ...
          <div className="absolute right-4 top-12 z-10">
              <DatePicker
                className="w-4  h-4 rounded-full bg-blue-500 text-blue-500 cursor-pointer"
                selected={betsSoftClose ? new Date(betsSoftClose) : null}
          ...
          <div className="absolute right-4 top-12 z-10">
              <DatePicker
                className="w-4  h-4 rounded-full bg-blue-500 text-blue-500 cursor-pointer"
                selected={betsHardClose ? new Date(betsHardClose) : null}
          ...
          <div className="mb-4 flex">
            <ButtonPlaceBet onClick={() => handleUpdateGame(game.id)}>
              Update game
            </ButtonPlaceBet>
          </div>
        </>
      ) : (
        <div className="text-cyan-500 py-8">Game updated successfully</div>
      )}
    </div>
  );
};

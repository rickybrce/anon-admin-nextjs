...
const SingleStatistic = ({ index, title, value, image }: Props) => {
    ...
    return (
        <div data-indx={index} className={`relative border-green-100 ${(index === 0 || index === 1 || index === 2) ? "border-b-[3px]" : "border-b-[3px] lg:border-none"}`}>
            <div  className={`relative flex flex-wrap justify-between border-green-100 my-[30px] py-[20px] px-[50px] ${(index !== 2 && index !== 5 && index !== 8) && "sm:border-r-[3px]"}`}>
                <div className='w-full 2xl:w-auto max-w-none xl:max-w-[140px] 2xl:max-w-[140px] 3xl:max-w-none mb-[16px] 2xl:mb-0'>
                    <div className='font-w-700 text-[20px] leading-[28px] lg:text-[18px] 2xl:text-[20px] text-white mb-1 min-w-[full]'>{title}</div>
